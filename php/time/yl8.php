...
foreach ($kuud as $number=>$nimetus){
    if ($number == $kuuNumber){
        echo $kuud[$number]; // kuvame eestikeelse kuu
    }
}
...
foreach ($nadal as $number=>$nimetus){
    if ($number == $paevaNumber){
        echo $nadal[$number]; // kuvame nädalapäeva
    }
}
...
// paneme jaanipäeva ajatempli, +1 iga aasta
$jargmineAasta = date('Y')+1;
$jaaniPaev = mktime(0, 0, 0, 6, 24, $jargmineAasta);
// paneme kirja hetkel oleva ajatempli
$hetkel = time();
...
$paevadeArv = (int)($vahe / $paevaSekunditeArv);
...
$minuSynnipaev = strtotime("6 September 1999".date('Y'));
$temaSynnipaev = strtotime("6 November 1980".date('Y'));
$vahe = $temaSynnipaev - $minuSynnipaev; //kui vahe on negatiivne - tema on vanem , kuna minu sünnipäev on hiljem
// sekundite arv aastas on 60 sekundit minutis * 60 minutit tunnis * 24 tundi * 30 päeva * 12 kuud
...
$jargmineAasta = date('Y')+1;
$minuSynniKuuPaev = strtotime("6 September 1999");
$minuSynnipaevJargmiselAastal = strtotime("6 September ".$jargmineAasta);
...
if ($vanusAastates % 5 == 0) {
    ...
    echo $jargmineAasta.' sul pole juublit';
}
